...
namespace wLib
{
    public partial class dbPanel : UserControl
    {
        ...
        // 설정 파일
        DB_CONF db_conf = new DB_CONF("LOCAL");
        ...
        public void Connect(DB_CONF _db_conf)
        {
            db_conf = _db_conf;
            ...
            // 데이터 바인딩
            db_BindingSource.DataSource = db_conf;
        ...
        public void ReadConfig()
        {
            ...
            db_BindingSource.ResetBindings(false);
        ...
        private async void On_Click_DB(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                using (wLib.DB.MYSQL_T conn = new wLib.DB.MYSQL_T(db_info))
                {
                    ...
                    temp = log.Info($"{button.Text}: {db_info.Ip}/{db_info.Dbname} 연결에 성공하였습니다.");
                    MessageBox.Show(temp);
                }
            }
            catch (Exception ex)
            {
                temp = log.Info($"{button.Text}: {db_info.Ip}/{db_info.Dbname} 연결에 실패하였습니다.\r\n{ex.Message}");
                MessageBox.Show(temp);
            }
